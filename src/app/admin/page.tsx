...
export default function AdminPage() {
  ...
  async function uploadCover(file: File): Promise<string | null> {
    ...
    if (!uid) { alert('Session habis, login lagi.'); return null; }
    const ext = (file.name.split('.').pop() || 'jpg').toLowerCase();
    const path = `${uid}/${Date.now()}.${ext}`;
    const { error: upErr } = await supabaseClient.storage.from('events')
      .upload(path, file, { upsert: false, cacheControl: '3600', contentType: file.type || undefined });
    if (upErr) { alert(`Upload gagal: ${upErr.message}`); return null; }
  ...
  async function onCreate() {
    ...
    if (file) {
      ...
      if (!cover_url) { setBusy(false); return; }
    }
    ...
    if ((res as any)?.error) { alert((res as any).error); setBusy(false); return; }

    // reset
    setTitle(''); setStartAt(''); setLocation(''); setDesc('');
    setIsPublished(false); setFile(null); setBusy(false);
    await loadEvents();
  }
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/90 backdrop-blur shadow-sm p-6">
          ...
          <form onSubmit={handleLogin} className="space-y-3 mt-5">
            <label className="block">
              ...
              <input className="w-full border border-slate-300 rounded-lg p-2 mt-1" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            </label>
            <label className="block">
              <span className="text-sm">Password</span>
              <input className="w-full border border-slate-300 rounded-lg p-2 mt-1" type="password" value={pass} onChange={(e) => setPass(e.target.value)} />
            </label>
            <button className="w-full py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 transition" type="submit">Sign in</button>
          </form>
        </div>
        ...
        <div className="mt-8 rounded-2xl border border-slate-200 p-5 bg-white shadow-sm">
          ...
          <div className="grid sm:grid-cols-2 gap-4 mt-3 text-sm">
            <ol className="list-decimal ml-5 space-y-1">
              ...
              <li>Isi form <b>Create Event</b>: Judul, Tanggal, (opsional) Lokasi & Deskripsi.</li>
              <li>Upload <b>Cover</b> (opsional) → klik <b>Save</b>.</li>
              <li>Gunakan tombol <b>Publish</b>/<b>Unpublish</b>.</li>
  ...
  return (
    <main className="mx-auto max-w-6xl p-8 text-slate-900 space-y-7">
      ...
      <header className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button onClick={handleLogout} className="px-3 py-2 rounded-lg border border-slate-300 hover:bg-slate-50">Logout</button>
      </header>
      ...
      <section className="grid sm:grid-cols-3 gap-4">
        {[
          ...
          <div key={c.label} className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-gradient-to-br from-emerald-50 to-white shadow-sm p-5">
        <div className="flex items-center justify-between">
          ...
          <a href="#create" className="text-sm text-emerald-700 hover:underline">Loncat ke form ↓</a>
        </div>
      ...
      {/* Create form */}
      <section id="create" className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6 space-y-4">
        <div className="font-medium">Create Event</div>
        <div className="grid md:grid-cols-2 gap-4">
          <label className="block">
            <span className="text-sm">Title</span>
            <input className="w-full border border-slate-300 rounded-lg p-2 mt-1" value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Contoh: Pelatihan UMKM Jambu" />
          </label>
          <label className="block">
            <span className="text-sm">Start time</span>
            <input className="w-full border border-slate-300 rounded-lg p-2 mt-1" type="datetime-local" value={startAt} onChange={(e) => setStartAt(e.target.value)} />
          </label>
          <label className="block">
            <span className="text-sm">Location (opsional)</span>
            <input className="w-full border border-slate-300 rounded-lg p-2 mt-1" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="Balai RW Gemblakan Atas..." />
          </label>
          <label className="block">
            <span className="text-sm">Cover (opsional)</span>
            <input className="w-full mt-1" type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
          ...
          <label className="block md:col-span-2">
            ...
            <textarea className="w-full border border-slate-300 rounded-lg p-2 mt-1 min-h-[96px]" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Ringkasan kegiatan, siapa yang boleh ikut, dsb." />
          </label>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={onCreate} disabled={busy} className="px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-60">
            {busy ? 'Saving…' : 'Save'}
          </button>
        </div>
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <input className="border border-slate-300 rounded-lg p-2 w-64" placeholder="Search title/location" value={q} onChange={(e) => setQ(e.target.value)} />
        </div>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="p-8 text-center text-slate-600">Tidak ada event. Buat baru di atas.</td></tr>
              ) : (
                filtered.map((e) => (
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-slate-600 truncate max-w-[36ch]">{e.description ?? ''}</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-3">
                      {new Date(e.start_at).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}
                      {e.location ? <div className="text-xs text-slate-600">{e.location}</div> : null}
                    </td>
                    <td className="p-3">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs border ${e.is_published ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>
                        {e.is_published ? 'Published' : 'Draft'}
                      </span>
                    </td>
                    <td className="p-3 text-right space-x-2">
                      <a href={`/events/${e.slug}`} target="_blank" className="px-3 py-1.5 rounded-lg border hover:bg-slate-50">View</a>
                      <button onClick={() => togglePublish(e.id, e.is_published)} className="px-3 py-1.5 rounded-lg border">
                        {e.is_published ? 'Unpublish' : 'Publish'}
                      </button>
                      <button onClick={() => remove(e.id)} className="px-3 py-1.5 rounded-lg border text-red-600">Delete</button>
                    </td>
                  </tr>
                ))
              )}
